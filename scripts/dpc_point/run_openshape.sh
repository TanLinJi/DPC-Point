...
for DATASET in "${DATASETS[@]}"; do
  ...
  for COMBINATION in "${COMBINATIONS[@]}"; do
    ...
    EXP_NAME="openshape_${DATASET}_${VARIANT}_${SEVERITY_SET}_e${ENTROPY_CAP}_g${GPA_CAP}_l${LOCAL_CAP}_n${NEG_CAP}_${SCORE_NAME}"

    bash "${SCRIPT_DIR}/run_common.sh" openshape "${DATASET}" "${GPU}" \
